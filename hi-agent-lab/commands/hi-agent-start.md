...
1. 如果 `$ARGUMENTS` 给了 lab id（如 `lab-01-attention`），直接调 `mcp__plugin_hi-agent-lab_hi-agent-lab__start_lab`；没给就先调 `mcp__plugin_hi-agent-lab_hi-agent-lab__list_labs`，用选项卡(AskUserQuestion)让学员选 lab / 先看看，再调 `mcp__plugin_hi-agent-lab_hi-agent-lab__start_lab`。
